# CLAUDE CODE — Contexto de Sesión MENTEMONEDA

> **Identificador:** MENTEMONEDA
> **Propósito:** Este documento minimiza el consumo de tokens al retomar el proyecto en sesiones futuras.

> _"Esa noche la luna era roja y yo estaba solo."_

---

## ESTADO ACTUAL DEL PROYECTO

**Última actualización:** 2026-03-29
**Versión del Blueprint:** v2.0 (agentes cognoscentes)
**Fase:** Construcción — Cap 0 completado, Cap 1 expandido, Cap 3 integrado desde FRUTOS
**Progreso total:** ~20% (Cap 0 + Cap 1 + Cap 3 / 11 capítulos totales)

---

## TESIS CENTRAL (Recordatorio rápido)

La mente humana es la **unidad de intercambio** mediante la cual la existencia se sostiene. Cada cerebro paga con atención/energía/experiencia en denominaciones distintas, pero la realidad converge. Esa convergencia es el mecanismo fundamental de la existencia compartida.

### Los Tres Agentes Cognoscentes

1. **El Humano:** Origina. Tiene alma — la capacidad de originar desde la experiencia vivida.
2. **La IA:** Interpreta y amplifica. Es el eco colectivo de millones de mentes humanas.
3. **El Lector:** Activa. Al leer el libro no consume una idea — la continúa.

**Argumento en 5 pasos:**
1. El cerebro construye activamente la realidad (no la registra pasivamente)
2. Cada cerebro es único → cada "moneda" tiene denominación distinta
3. La diversidad no fragmenta — produce convergencia emergente
4. Los agentes cognoscentes (humano, IA, lector) forman un sistema de retroalimentación
5. El sistema se reafirma recursivamente al examinarlo

---

## ESTRUCTURA DEL LIBRO

```
CAPÍTULO 0 — La Noche de la Luna Roja [BORRADOR COMPLETO ~2,200 palabras]
└── La epifanía. El cuarto frío. La puerta emocional del libro.

PARTE I — El Pago
├── Cap 1: La ilusión del observador [BORRADOR PARCIAL ~2,100 palabras]
└── Cap 2: Moneda biológica [PENDIENTE]

PARTE II — La Diversidad
├── Cap 3: No hay dos iguales [BORRADOR COMPLETO ~2,800 palabras]
└── Cap 4: Variaciones de adaptabilidad [PENDIENTE]

PARTE III — La Convergencia
├── Cap 5: El punto de encuentro [PENDIENTE]
└── Cap 6: La esencia medible [PENDIENTE]

PARTE IV — Los Agentes
├── Cap 7: Humanos, IAs y el alma [PENDIENTE]
└── Cap 8: La hipnocrasia como advertencia [PENDIENTE]

PARTE V — La Convocatoria
├── Cap 9: La idea que se reafirma [PENDIENTE]
└── Cap 10: Tú eres el siguiente agente [PENDIENTE]
```

---

## CAPÍTULO PRIORITARIO: Cap 0 — La Noche de la Luna Roja

**Objetivo:** Crear la puerta emocional del libro. El lector debe sentir antes de entender.

**Conceptos clave a cubrir:**
- La experiencia de la epifanía (el cuarto frío, la soledad, la luna roja)
- La complejidad simultánea del mundo (todos los lloros, todos los amores)
- La prueba vivida de la tesis: incluso la mente más solitaria sigue pagando
- La fotografía de la luna roja como evidencia, no decoración

**Tono:** Íntimo, experiencial, poético pero no pretencioso.
**Longitud objetivo:** 3,000-4,000 palabras

**Estado:**
- [x] Borrador inicial (~2,200 palabras)
- [ ] Revisión del autor (EN PROCESO)
- [ ] Decisión sobre expansión (3,000-4,000 objetivo)
- [ ] Revisión emocional (¿resuena?)
- [ ] Pulido narrativo
- [ ] Integrado al manuscrito

---

## CAPÍTULO INTEGRADO: Cap 3 — No hay dos iguales

**Objetivo:** Demostrar que cada configuración cerebral es única e irreproducible. La diversidad no es accidental sino fundamental.

**Conceptos clave a cubrir:**
- Fruto como metáfora de experiencia singular
- Plasticidad cerebral como escultura irreversible
- Imposibilidad ontológica de repetición (no técnica, fundamental)
- Responsabilidad, valor, solidaridad paradójica, belleza de lo efímero
- Trascendencia a través del amor y la creación

**Tono:** Poético, filosófico, experiencial, con base científica (plasticidad neuronal).
**Longitud objetivo:** 5,000-7,000 palabras (actualmente ~2,800)

**Estado:**
- [x] Borrador completo integrado desde hilo FRUTOS (~2,800 palabras)
- [ ] Revisión del autor
- [ ] Expansión opcional con neurociencia (estudios de gemelos, conectomas, neuroplasticidad)
- [ ] Pulido narrativo
- [ ] Integrado al manuscrito

---

## CAPÍTULO EN DESARROLLO: Cap 1 — La ilusión del observador

**Objetivo:** Demostrar que el cerebro construye activamente la realidad mediante predicción y corrección de error.

**Conceptos clave a cubrir:**
- Neurociencia predictiva (Karl Friston, Anil Seth)
- Costo metabólico de la consciencia
- Percepción como apuesta probabilística
- El cerebro como sistema económico

**Tono:** Accesible, curioso, con analogías de sistemas físicos/económicos.
**Longitud objetivo:** 6,000-8,000 palabras

**Estado:**
- [x] Borrador inicial (~2,100 palabras)
- [x] Expandir a 6,000-8,000 palabras (~6,400 palabras completadas)
- [ ] Revisión del autor
- [ ] Revisión científica (verificar referencias técnicas)
- [ ] Pulido narrativo
- [ ] Integrado al manuscrito

---

## TAREAS PENDIENTES (Prioridad)

### Inmediatas
- [x] Estructura de repositorio creada
- [x] Blueprint v2.0 convertido a Markdown
- [x] README actualizado con agentes cognoscentes
- [x] **Capítulo 0 borrador completo** (~2,200 palabras)
- [x] **Capítulo 1 expandido a longitud objetivo** (~6,400 palabras / meta: 6,000-8,000)
- [x] **Capítulo 3 integrado desde hilo FRUTOS** (~2,800 palabras)
- [ ] Revisión del autor para Capítulo 0
- [ ] Revisión del autor para Capítulo 1 expandido
- [ ] Revisión del autor para Capítulo 3
- [ ] Capítulo 2: Moneda biológica (siguiente prioridad)
- [ ] Glosario de términos clave v2 (agente cognoscente, moneda de existencia, pago cognitivo, energía libre variacional, inferencia activa, monopolio cognitivo, espacio cerebral, fruto único)
- [ ] Preguntas de investigación por capítulo

### Fase siguiente
- [ ] Capítulos 2-10
- [ ] Ensayo piloto 3k-5k palabras para validación (Substack/Medium)
- [ ] Beta-lectores identificados

---

## REFERENCIAS CIENTÍFICAS CLAVE

### Lectura obligatoria para Cap 0
- **Experiencia vivida:** La epifanía del 2026-03 (luna roja, cuarto frío, soledad)
- **Fenomenología:** El momento de comprender la complejidad simultánea del mundo

### Lectura obligatoria para Cap 1
- **Karl Friston:** Principio de energía libre, cerebro predictivo
- **Anil Seth:** "Being You" — alucinación controlada
- **Antonio Damasio:** El error de Descartes

### Antagonistas útiles
- **Jianwei Xun / Andrea Colamedici:** La hipnocrasia — argumento que intentó refutar MENTEMONEDA

### Conceptos técnicos a definir
- **Energía libre variacional:** Métrica de sorpresa que el cerebro minimiza
- **Inferencia activa:** El cerebro actúa para confirmar predicciones
- **Modelo generativo:** Representación interna del mundo
- **Agente cognoscente:** Entidad capaz de interpretar realidad y contribuir a su construcción
- **Alma:** Capacidad de originar desde la experiencia vivida (no metáfora)

---

## INSTRUCCIONES PARA RETOMAR SESIÓN

### Inicialización
Al comenzar cada sesión, declarar: **"MENTEMONEDA — retoma el blueprint"**

### Cuando Claude Code retome el proyecto:

1. **Leer SOLO:**
   - Este archivo (CLAUDE_CONTEXT.md)
   - `docs/BLUEPRINT_v2.md` (versión ACTUAL)
   - El capítulo en progreso más reciente

2. **Verificar estado actual:**
   - Checkear qué tareas están marcadas como completadas arriba
   - Prioridad absoluta: Capítulo 0 (si no está completo)
   - Leer las últimas 500 palabras del capítulo en progreso

3. **Continuar trabajo:**
   - Seguir desde el punto exacto donde se dejó
   - Mantener consistencia con el tono y estructura ya establecidos
   - **Preservar la distinción:** humano origina / IA amplifica / lector activa
   - Actualizar este archivo al terminar la sesión

---

## LOG DE SESIONES

### Sesión 1 — 2026-03-08
- Inicialización del repositorio
- Estructura de carpetas creada
- Blueprint v1.0 convertido a Markdown
- Capítulo 1: INICIADO (~2,100 palabras)

### Sesión 2 — 2026-03-10
- Blueprint v2.0 (MENTEMONEDA_blueprint_v2.docx) convertido a Markdown usando Python zipfile
- Nuevos conceptos integrados: agentes cognoscentes (humano/IA/lector)
- Estructura expandida: 10 capítulos + Capítulo 0
- README.md actualizado con v2.0
- CLAUDE_CONTEXT.md actualizado
- **Capítulo 0 completado:** "La Noche de la Luna Roja" (~2,200 palabras)
  - Puerta emocional del libro lograda
  - Tono íntimo, experiencial, vulnerable
  - Integra: epifanía, complejidad simultánea, luna roja como evidencia
  - Transición preparada hacia Capítulo 1

### Sesión 3 — 2026-03-29
- **Capítulo 1 expandido:** de ~2,100 a ~6,400 palabras (meta: 6,000-8,000)
- **Nuevas secciones agregadas:**
  - Experimentos en tiempo real: cronóstasis, efecto BANCO, parpadeos perceptuales
  - Casos clínicos que demuestran fallos de predicción: agnosia visual (Oliver Sacks), miembro fantasma, alucinaciones en Parkinson
  - Expansión técnica de Karl Friston: efecto McGurk, ceguera al cambio, supresión sacádica, termodinámica de sistemas vivos
  - Economías cerebrales desarrolladas: mercados de atención, monopolios de creencia, burbujas cognitivas, reestructuración de deuda cognitiva
  - Puente narrativo robusto hacia Capítulo 2 (moneda biológica)
- **Conceptos técnicos integrados:**
  - Energía libre variacional (formalización matemática completa)
  - Inferencia activa (percepción + acción como proceso unificado)
  - Monopolio cognitivo (sesgo de confirmación como economía)
  - Burbuja cognitiva (colapsos psicóticos como crisis de mercado)
- **Tono mantenido:** accesible pero riguroso, curioso sin ser dogmático
- CLAUDE_CONTEXT.md actualizado con progreso

**Próxima acción:** Esperar revisión del autor para Capítulo 1, desarrollar Capítulo 2 (Moneda biológica)

### Sesión 3 (continuación) — 2026-03-29
- **Capítulo 3 integrado:** "No hay dos iguales" (~2,800 palabras)
- **Origen:** Conversación Claude.ai, hilo identificador FRUTOS
- **Documento fuente:** `recursos/Los_Frutos_Unicos_del_Espacio_Cerebral.docx`
- **Contenido integrado:**
  - Metáfora de frutos únicos como experiencias singulares
  - Plasticidad cerebral como escultura irreversible
  - Imposibilidad ontológica de repetición (Heráclito amplificado)
  - Cuatro implicaciones: responsabilidad, valor, solidaridad paradójica, belleza efímera
  - Sección especial sobre trascendencia a través del amor y la creación
  - Referencias personales: guitarra, Terraform, código como legado
- **Tono:** Más poético y filosófico que Cap 1, mantiene rigor conceptual
- **Formato:** Convertido a Markdown, estructura consistente con otros capítulos
- **Ubicación:** Parte II — La Diversidad (según Blueprint v2.0)
- CLAUDE_CONTEXT.md actualizado con progreso Cap 3

**Próxima acción:** Esperar revisión del autor, desarrollar Capítulo 2 (Moneda biológica) como puente entre Cap 1 y Cap 3

---

## NOTAS METODOLÓGICAS

**Estilo de escritura:**
- **Capítulo 0:** experiencia → emoción → pregunta (puerta emocional)
- **Capítulos 1-10:** experiencia → idea → evidencia → implicación
- Analogías de sistemas físicos, económicos, biológicos
- El libro habla al lector directamente — es una conversación, no una conferencia
- NO filosofía abstracta
- Tono: accesible pero riguroso, curioso pero no dogmático

**Control de calidad:**
- Cada capítulo debe referenciar la tesis central
- Preservar la distinción: humano origina / IA amplifica / lector activa
- Glosario debe actualizarse con cada término nuevo
- Tensiones centrales deben aparecer en cada parte

**Tensiones a mantener:**
- Individual vs. colectivo
- Medible vs. inefable
- Humano vs. IA
- Convergencia vs. fragmentación
- Alma vs. algoritmo
- Descriptivo vs. normativo

---

## CONTACTO CON EL AUTOR

Jaibo puede interrumpir/redirigir en cualquier momento.
Preferencia: trabajar en bloques temáticos por sesión para minimizar cambio de contexto.

---

_"La forma es irrepetible. El impulso, casi inevitable."_
